...
// @access  Private
router.get('/profile', auth, async (req, res) => { // Ensure 'async' is present
    try {
...
// @access  Private
router.put('/profile', auth, async (req, res) => { // Ensure 'async' is present
...
// @access  Private
router.get('/match', auth, async (req, res) => { // Ensure 'async' is present
    try {
        ...
        console.error(err.message); // Optional: Log the error for debugging
        res.status(500).json({ message: 'Server error' });
    }
});
